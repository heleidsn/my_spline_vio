...
void FullSystem::makeKeyFrame( FrameHessian* fh)
{
	...
	allKeyFramesHistory.push_back(fh->shell);   //!< 将关键帧的shell加入到历史关键帧中进行保存
	ef->insertFrame(fh, &Hcalib);               //!< 将关键帧加入到优化中

	setPrecalcValues();                         //!< 设置预计算值



	// =========================== add new residuals for old points =========================
	int numFwdResAdde=0;
	for(FrameHessian* fh1 : frameHessians)		//!< 遍历所有的帧
	{
		if(fh1 == fh) continue;
		for(PointHessian* ph : fh1->pointHessians)  //!< 遍历所有的点
        {
		...
		{
			PointFrameResidual* r = new PointFrameResidual(ph, fh1, fh);  //!< 这咋和上面一毛一样
			r->setState(ResState::IN);
			ph->residuals.push_back(r);
			ef->insertResidual(r);
			ph->lastResiduals[1] = ph->lastResiduals[0];
	...
	activatePointsMT();  //!< 激活点
	ef->makeIDX();       //!< 重新编号  ef是新建的EnergyFunction对象
	...
	float rmse = optimize(setting_maxOptIterations);  // 重要！！！全局的更新 setting_maxOptIterations=6
...
void FullSystem::printLogLine()
{
    ...
    if(!setting_debugout_runquiet)
        printf("LOG %d: %.3f fine. Res: %d A, %d L, %d M; (%'d / %'d) forceDrop. a=%f, b=%f. Window %d (%d)\n",
                allKeyFramesHistory.back()->id,  // key frame id
                statistics_lastFineTrackRMSE,    // RMSE
                ef->resInA, // 
                ef->resInL,
                ef->resInM,
