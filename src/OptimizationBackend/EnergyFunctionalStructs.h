...
public:
  ...
  EFPoint *point;           //!< 残差对应的点
  EFFrame *host;            //!< 残差对应的帧
  EFFrame *target;          //!< 残差对应的目标帧
  int idxInAll;

  RawResidualJacobian *J;   //!< 残差对应的雅克比

  VecNRf res_toZeroF;       //!< 更新delta后的线性残差
  Vec8f JpJdF;              //!< 逆深度Jaco和位姿+光度Jaco的Hessian

  // status.
  bool isLinearized;        //!< 是否已经线性化
...
public:
  ...
  EFFrame *host;   //! contains all residuals.
...
public:
  ...
  Vec8 prior;       // prior hessian (diagonal)  //!< 位姿只有第一帧有先验 prior hessian (diagonal)
  Vec8 delta_prior; // = state-state_prior (E_prior = (delta_prior)' *  //!< 相对于先验的增量
                    // diag(prior) * (delta_prior)
  Vec8 delta;       // state - state_zero.  //!< 相对于线性化点位姿, 光度的增量

  std::vector<EFPoint *> points;  //!< 帧上所有点
  FrameHessian *data;             //!< 对应FrameHessian数据
  int idx; // idx in frames.      //!< 在能量函数中帧id // idx in frames.
